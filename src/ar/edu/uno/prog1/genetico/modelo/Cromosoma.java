...
public class Cromosoma {
	...
	public Cromosoma(String representacionGenesHexadecimal) {
		...
		this.setRepresentacionGenesBinaria(this
				.getConvertirHexadecimalABinario());
	}

	private String getConvertirHexadecimalABinario() {
		String temp = null;
		temp = Long.toBinaryString(Long.parseLong(
				this.getRepresentacionGenesHexadecimal(), 16));
	...
	private String getConvertirBinarioAHexadecimal() {
		...
		String temp;
		temp = Long.toHexString(
				Long.parseLong(this.getRepresentacionGenesBinaria(), 2))
				.toUpperCase();
	...
	public Integer calcularFitness(Cromosoma clave) {
		return (this.getCantidadHexadecimalCoincidentes(clave) + 1)
				* (int) (Math.pow(this.getCantidadBinariosCoincidentes(clave),
						2));
	}

	private Integer getCantidadHexadecimalCoincidentes(Cromosoma clave) {
		Integer h = 0;
		for (Integer i = 0; i < this.getRepresentacionGenesHexadecimal()
				.length(); i++)
			if (this.getRepresentacionGenesHexadecimal().charAt(i) == clave
					.getRepresentacionGenesHexadecimal().charAt(i))
				h++;
		return h;
	}
	...
	private Integer getCantidadBinariosCoincidentes(Cromosoma clave) {
		...
		for (Integer i = 0; i < this.getRepresentacionGenesBinaria().length(); i++)
			if (this.getRepresentacionGenesBinaria().charAt(i) == clave
					.getRepresentacionGenesBinaria().charAt(i))
				b++;
		return b;
	}
	...
	public void setRepresentacionGenesBinaria(String representacionGenesBinaria) {
		...
		this.representacionGenesHexadecimal = Long.toHexString(
				Long.parseUnsignedLong(this.representacionGenesBinaria, 2))
				.toUpperCase();
	}
	...
	public void mutar() {
		...
		this.setRepresentacionGenesHexadecimal(this
				.getConvertirBinarioAHexadecimal());
	}
	...
	public Cromosoma[] reproducir(Cromosoma pareja) {
		...
		Cromosoma hijo1 = new Cromosoma(
				this.getRepresentacionGenesHexadecimal());
